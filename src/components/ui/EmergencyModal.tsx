    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs">
        ...
        >
          ...
          <div className="p-5 sm:p-6 overflow-y-auto space-y-4 text-[#1F2A2A]">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#FFF5F5] border-2 border-[#D97A7A] rounded-2xl p-4 text-center">
                ...
                <span className="text-3xl sm:text-4xl font-black text-[#1F2A2A] block mt-0.5">
                  {emergencyProfile.bloodGroup.split(' ')[0]}
                </span>
                <span className="text-[11px] text-[#6C7A7A]">Rh Positive</span>
              </div>

              <div className="bg-[#F7F8F7] border border-[#E2EAE8] rounded-2xl p-4 flex flex-col justify-center">
                <span className="text-xs text-[#6C7A7A] block">Organ Donor</span>
                <span className="text-base font-bold text-[#2F7E79] flex items-center gap-1.5 mt-0.5">
                  <Heart className="w-4 h-4 text-[#2F7E79]" />
                  Yes (Registered)
                </span>
                <span className="text-[11px] text-[#6C7A7A] mt-1">
                  Conditions: {emergencyProfile.chronicConditions[0] || 'None'}
                </span>
              </div>
            </div>
            ...
            <div className="bg-[#FFF5F5] border-2 border-[#D97A7A] rounded-2xl p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {emergencyProfile.severeAllergies.map((all, i) => (
                  <span
                    key={i}
                    className="bg-white border border-[#D97A7A] text-[#D97A7A] text-sm font-bold px-3 py-1 rounded-xl shadow-xs"
                  >
                    {all}
                  </span>
                ))}
            ...
            <div className="space-y-2">
              <span className="text-xs font-bold uppercase tracking-wider text-[#6C7A7A]">
                Emergency Contacts
              </span>
              <div className="space-y-2">
                {emergencyProfile.emergencyContacts.map((contact, i) => (
                  <div
                    key={i}
                    className="bg-[#F7F8F7] border border-[#E2EAE8] rounded-2xl p-3.5 flex items-center justify-between"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {contact.isPrimary && (
                          <span className="text-[10px] bg-[#EEF3F2] text-[#2F7E79] font-bold px-2 py-0.5 rounded-full">
                            Primary
                          </span>
                        )}
                      </div>
                    ...
                    <a
                      ...
                      className="touch-target inline-flex items-center gap-1.5 bg-[#2F7E79] hover:bg-[#266864] text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-xs"
                    >
            ...
            {/* Active Medications */}
            <div className="bg-[#F7F8F7] border border-[#E2EAE8] rounded-2xl p-3.5">
              ...
              <div className="flex flex-wrap gap-1.5 text-xs font-medium text-[#1F2A2A]">
                {emergencyProfile.activeMedications.map((m, i) => (
                  <span
                    key={i}
                    className="bg-white border border-[#E2EAE8] px-2.5 py-1 rounded-lg"
                  >
                    {m}
                  </span>
                ))}
            ...
            {/* Directive note */}
            <div className="text-[11px] text-[#6C7A7A] bg-[#EEF3F2] p-3 rounded-xl">
              <span className="font-bold text-[#1F2A2A]">Directives: </span>
              {emergencyProfile.medicalDirectives}
            </div>
          </div>

          {/* Bottom Actions */}
          <div className="p-4 border-t border-[#E2EAE8] bg-[#F7F8F7] flex items-center justify-between">
            <button
              onClick={handleCopySummary}
              className="touch-target inline-flex items-center gap-1.5 text-xs font-bold text-[#1F2A2A] bg-white border border-[#E2EAE8] hover:bg-[#F7F8F7] px-4 py-2 rounded-xl transition-all"
            >
              {copied ? <Check className="w-3.5 h-3.5 text-[#74A57F]" /> : <Copy className="w-3.5 h-3.5" />}
              <span>{copied ? 'Copied' : 'Copy Summary'}</span>
            </button>

            <button
              onClick={() => setIsEmergencyModalOpen(false)}
              className="touch-target px-6 py-2 rounded-xl bg-[#2F7E79] text-white text-xs font-bold"
            >
              Done
            </button>
          </div>
